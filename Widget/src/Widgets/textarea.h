...
class WTextArea : public Widget{
    ...
    public : 
        WTextArea(lv_obj_t * display = nullptr, int pos_x = 0, int pos_y = 0, int size_x = 200, int size_y = 50, bool hiden = false, const char* text = "", const char * placeholder_text = "", void (*f)(lv_event_t *) = default_func);
        void draw();
        ...
        void triggerCallback(lv_event_t * event) {this->callback(event);}
};
