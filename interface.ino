...
void loop() {
  lv_timer_handler();  // Gestion de l'affichage

  bool currently_connected = msd->connect();

  // Detection d'insertion USB
  ...
  // Detection retrait USB
  if (!currently_connected && usb_connected) {
    usb_connected = false;

    if (usb_mounted) {
      usb.unmount();  // Demonter proprement
      usb_mounted = false;
      Serial.println("USB demontee proprement.");
    }

    // Supprimer et recreer l'objet msd pour forcer une reinitialisation
    delete msd;
    msd = new USBHostMSD();
  ...
  // Si la cle est branchee mais non montee, tenter un montage après 5 secondes
  if (usb_connected && !usb_mounted && (millis() - connect_timer > 5000)) {
    ...
    if (err) {
      ...
      usb_mounted = true;
      Serial.println("Cle USB montee avec succès.");

      // Lecture recursive des fichiers XML
      char buf[256];
      DIR* d = opendir("/usb/");
      if (!d) {
...
// Parcours recursif des fichiers XML
void listFilesRecursive(const char* dirPath, unsigned int& count) {
  ...
  while ((e = readdir(d)) != nullptr) {
    ...
    if (e->d_type == DT_DIR) {
      ...
      count++;
      snprintf(buf, sizeof(buf), "Fichier trouve : %s", e->d_name);
      Serial.println(buf);
    }
  }
